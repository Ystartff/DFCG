...
from lib.decoders import CUP, CASCADE, CASCADE_Cat, GCUP, GCUP_Cat, GCD, GCD_Cat
...
class PVT_GCD(nn.Module):
    def __init__(self, n_class=1, img_size=224, k=11, padding=5, conv='mr', gcb_act='gelu', activation='relu', skip_aggregation='additive'):
        super(PVT_GCD, self).__init__()
        ...
        # decoder initialization
        if self.skip_aggregation == 'additive':
            self.decoder = GCD(channels=self.channels, img_size=img_size, k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
        elif self.skip_aggregation == 'concatenation':
            self.decoder = GCD_Cat(channels=self.channels, img_size=img_size, k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
            self.channels = [self.channels[0], self.channels[1]*2, self.channels[2]*2, self.channels[3]*2]
        else:
            print('No implementation found for the skip_aggregation ' + self.skip_aggregation + '. Continuing with the default additive aggregation.')
            self.decoder = GCD(channels=self.channels, img_size=img_size, k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
        ...
        print('Model %s created, param count: %d' %
                     ('GCD decoder: ', sum([m.numel() for m in self.decoder.parameters()])))
...
class MERIT_GCD(nn.Module):
    def __init__(self, n_class=1, img_size_s1=(256,256), img_size_s2=(224,224), k=11, padding=5, conv='mr', gcb_act='gelu', activation='relu', interpolation='bilinear', skip_aggregation='additive'):
        ...
        # decoder initialization 
        if self.skip_aggregation == 'additive':
            self.decoder1 = GCD(channels=self.channels, img_size=img_size_s1[0], k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
            self.decoder2 = GCD(channels=self.channels, img_size=img_size_s2[0], k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
        elif self.skip_aggregation == 'concatenation':
            self.decoder1 = GCD_Cat(channels=self.channels, img_size=img_size_s1[0], k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
            self.decoder2 = GCD_Cat(channels=self.channels, img_size=img_size_s2[0], k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
            self.channels = [self.channels[0], self.channels[1]*2, self.channels[2]*2, self.channels[3]*2]
        else:
            print('No implementation found for the skip_aggregation ' + self.skip_aggregation + '. Continuing with the default additive aggregation.')
            self.decoder1 = GCD(channels=self.channels, img_size=img_size_s1[0], k=k, padding=padding, conv=conv, gcb_act=gcb_act, activation=activation)
        ...
        print('Model %s created, param count: %d' %
                     ('GCD decoder: ', sum([m.numel() for m in self.decoder1.parameters()])))
        print('Model %s created, param count: %d' %
                     ('GCD decoder: ', sum([m.numel() for m in self.decoder2.parameters()])))
